CREATE OR REPLACE PACKAGE relatorios_habitacao_pkg AS
  PROCEDURE RELATORIO_HABITACAO_POR_FACCAO(p_cpi_lider CHAR, p_data_ini DATE, p_data_fim DATE);
  PROCEDURE RELATORIO_HABITACAO_POR_ESPECIE(p_cpi_lider CHAR, p_data_ini DATE, p_data_fim DATE);
  PROCEDURE RELATORIO_HABITACAO_POR_PLANETA(p_cpi_lider CHAR, p_data_ini DATE, p_data_fim DATE);
  PROCEDURE RELATORIO_HABITACAO_POR_SISTEMA(p_cpi_lider CHAR, p_data_ini DATE, p_data_fim DATE);
  PROCEDURE EXECUTAR_RELATORIO(opcao VARCHAR2, p_param1 VARCHAR2, p_data_ini DATE, p_data_fim DATE);
END relatorios_habitacao_pkg;


CREATE OR REPLACE PACKAGE BODY relatorios_habitacao_pkg AS

  PROCEDURE RELATORIO_HABITACAO_POR_FACCAO(p_cpi_lider CHAR, p_data_ini DATE, p_data_fim DATE) IS
    v_nacao VARCHAR2(15);
  ...
  BEGIN
    ...
    END IF;
  END RELATORIO_HABITACAO_POR_FACCAO;

  PROCEDURE RELATORIO_HABITACAO_POR_ESPECIE(p_cpi_lider CHAR, p_data_ini DATE,p_data_fim DATE) IS
  v_nacao VARCHAR2(15);
  v_total_habitantes_atual NUMBER := 0;
  v_total_comunidades_atual NUMBER := 0;
...
EXCEPTION
  ...
  END RELATORIO_HABITACAO_POR_ESPECIE;

  PROCEDURE RELATORIO_HABITACAO_POR_PLANETA(p_cpi_lider CHAR, p_data_ini DATE, p_data_fim DATE) IS
  v_nacao VARCHAR2(15);
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('Erro: ' || SQLERRM);
  END RELATORIO_HABITACAO_POR_PLANETA;

  PROCEDURE RELATORIO_HABITACAO_POR_SISTEMA(p_cpi_lider CHAR, p_data_ini DATE, p_data_fim DATE) IS
  v_nacao VARCHAR2(15);
  v_total_habitantes NUMBER := 0;
  v_media_habitantes NUMBER := 0;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('Erro: ' || SQLERRM);
  END RELATORIO_HABITACAO_POR_SISTEMA;


  PROCEDURE EXECUTAR_RELATORIO(opcao VARCHAR2, p_param1 VARCHAR2, p_data_ini DATE, p_data_fim DATE) IS
  BEGIN
    CASE opcao
      WHEN 'FACCAO' THEN
        RELATORIO_HABITACAO_POR_FACCAO(p_param1, p_data_ini, p_data_fim);
      WHEN 'ESPECIE' THEN
        RELATORIO_HABITACAO_POR_ESPECIE(p_param1, p_data_ini, p_data_fim);
      WHEN 'PLANETA' THEN
        RELATORIO_HABITACAO_POR_PLANETA(p_param1, p_data_ini, p_data_fim);
      WHEN 'SISTEMA' THEN
        RELATORIO_HABITACAO_POR_SISTEMA(p_param1, p_data_ini, p_data_fim);
      ELSE
        DBMS_OUTPUT.PUT_LINE('Opção inválida. Selecione uma opção de 1 a 4.');
    END CASE;
  END EXECUTAR_RELATORIO;
